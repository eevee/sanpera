...
def test_crop(ctx):
    img = builtins.rose.crop(Size(40, 30).at((10, 10)), preserve_canvas=True)
    ctx.compare(img, 'crop.miff')

@convert('convert rose: -crop 40x30+40+30 crop_br.miff')
def test_crop_br(ctx):
    img = builtins.rose.crop(Size(40, 30).at((40, 30)), preserve_canvas=True)
...
def test_crop_all(ctx):
    img = builtins.rose.crop(Size(90, 60).at((-10, -10)), preserve_canvas=True)
    ctx.compare(img, 'crop_all.miff')

@convert('convert rose: -crop 40x30+90+60 crop_miss.miff')
def test_crop_miss(ctx):
    img = builtins.rose.crop(Size(40, 30).at((90, 60)), preserve_canvas=True)
